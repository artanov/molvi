...
Статус: **порт завершён и выпущен** (2026-07-10, сессия на маке):
релиз **v1.3.0** с DMG, диктовка проверена владельцем вживую, сайт обновлён
(install-mac.html, /geek/mac, «M1, M2, M3, M4 и новее» на главной).
Железо разработки: MacBook Pro M1, **8 ГБ RAM**, macOS 15.3.
Написано сессией на Windows-машине 2026-07-10; ниже — журнал порта и грабли.
...
Живая проверка диктовки владельцем на M1 пройдена (2026-07-10): хоткей →
запись → распознавание → вставка работают end-to-end.

**Грабля Gatekeeper в macOS 15 (Sequoia)**: Apple убрала «правый клик →
Открыть» для ненотаризованных приложений — диалог предлагает только
«Переместить в Корзину». Рабочий путь: «Готово» → System Settings →
Конфиденциальность и безопасность → «Открыть всё равно», либо
`xattr -cr /Applications/Molvi.app`. Инструкции (сайт, README) переписаны
2026-07-12; поймано владельцем при чистой установке релизного DMG.
Полное удаление для переустановки «с нуля» — FAQ на install-mac.html
(приложение, Application Support, LaunchAgent, кэш модели, tccutil reset).

**Грабля TCC при переустановке**: подпись сборки ad-hoc и меняется при
каждой пересборке — macOS молча аннулирует выданные «Мониторинг ввода» и
«Универсальный доступ» (симптом: tap создаётся, но enabled=False в
CGGetEventTapList; вставка падает с нашей понятной OSError). Лечение:
в System Settings выключить-включить обе галочки Molvi (или удалить/добавить
приложение в списке) и перезапустить. Уйдёт со стабильной подписью:
самоподписанный сертификат в build-mac.sh (бесплатно) или Developer ID +
нотаризация ($99/год) — решение владельца.

Сделано после релиза v1.3.0: страница molvi.tech/install-mac.html,
гик-версия molvi.tech/geek/mac (Apple-стиль), мак на главной. Локальные
сборки подписываются «Molvi Dev Signing» (самоподписанный в keychain этого
мака; создание — в сообщении коммита ff45f3e) — разрешения TCC переживают
обновления. Релизный DMG из CI — ad-hoc: у пользователей разрешения слетают
при обновлении версии (FAQ на сайте предупреждает). Открыто владельцу:
положить сертификат секретом в GitHub Actions или купить Developer ID
($99/год — заодно уйдёт Gatekeeper-предупреждение).
